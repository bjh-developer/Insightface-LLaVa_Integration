'''
vis max_sim threshold.py takes one image from VIS image folder to be the template image for comparison.
It then runs through all the other images in VIS image folder that does not contain the person in the template image as test images.
...
template_file = "s1_VIS_00001_001.jpg"
...
onlyfiles = [f for f in listdir(mypath) if isfile(join(mypath, f)) and 'VIS_00001' not in f]
print(onlyfiles)
...
plt.savefig('treshhold_for_VIS_image.png')
